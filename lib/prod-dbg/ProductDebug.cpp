...
public:
  ...
  void execute(unsigned int argc, const char **args, unsigned int idxToFirstArgToHandle)
  {
    ...
    Serial.println();
    Serial.println(wlStatus == WL_NO_SHIELD ? "NO_SHIELD      " : wlStatus == WL_IDLE_STATUS   ? "IDLE_STATUS    "
...
public:
  ...
  void execute(unsigned int argc, const char **args, unsigned int idxToFirstArgToHandle)
  {
    ...
    else
    {
      ...
      Serial.println(status == 200 ? "OK / Success                                                                           " : status == 404 ? "Incorrect API key (or invalid ThingSpeak server address)                               "
                                                                                                                             ...
                                                                                                                             : status == -401  ? "Point was not inserted (most probable cause is the rate limit of once every 15 seconds)"
                                                                                                                                               : "UNKNOWN");
    }
  }
